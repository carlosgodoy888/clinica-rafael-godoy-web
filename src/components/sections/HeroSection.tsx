...
export function HeroSection() {
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-gradient-to-br from-teal-900 via-teal-700 to-teal-600"
      />
      <div
        aria-hidden="true"
      ...
      <div
        aria-hidden="true"
        className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full bg-teal-500/15 blur-3xl -translate-y-1/3 translate-x-1/4"
      />
      <div
        aria-hidden="true"
        className="absolute bottom-0 left-0 w-96 h-96 rounded-full bg-teal-900/40 blur-3xl translate-y-1/3 -translate-x-1/4"
      />
      ...
      <Container className="relative z-10 flex-1 flex items-center pt-20 pb-12 md:pt-28 md:pb-16">
        <div className="w-full max-w-3xl mx-auto text-center">
          ...
          <div className="mt-5 inline-flex flex-wrap items-center justify-center gap-3">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full border border-white/15">
              <div
                ...
                aria-label="Valoración media 5 estrellas"
                role="img"
              >
                {[...Array(5)].map((_, i) => (
                  <Star
          ...
          <p className="mt-5 text-sm md:text-base text-white/70 leading-relaxed max-w-2xl mx-auto">
            ...
            . Tratamientos personalizados de ortodoncia, implantes dentales,
            endodoncia, estética dental, limpieza, periodoncia y prótesis.
          </p>

          {/* CTAs principales con tracking */}
          <HeroCtaButtons />
            ...
            <dl className="flex flex-wrap justify-center gap-6 md:gap-14">
              {trustStats.map(({ value, label }) => (
                <div key={label} className="text-center">
                  ...
                  <dd className="mt-0.5 text-xs sm:text-sm text-teal-200">
                    {label}
                  </dd>
                </div>
              ))}
      ...
      <div
        aria-hidden="true"
        className="absolute bottom-0 left-0 right-0 z-10 leading-none"
      >
        <svg
          viewBox="0 0 1440 72"
